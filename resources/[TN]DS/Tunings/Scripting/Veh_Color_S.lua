Marker1 = createMarker(870.48890625 ,-1169.8646875 ,16.37164901,"cylinder",6,0,0,0,0)
Marker2 = createMarker(858.0256640625 ,-1169.8646875 ,16.37164901,"cylinder",6,0,0,0,0)
local accSys = exports["Accounts-System"]
local vehSys = exports["[tn]DS"]
function shownotf( yaru )
	if getElementType ( yaru ) == "player" then
		if isPedInVehicle (yaru) then
			exports["notf"]:addNotification(yaru,"Baraye Taghir Rang Mashin Khod Az /vehcolor Estefade Konid!", 'info')
			triggerClientEvent ( yaru, "Playinfo", yaru )
		end
	end
end
addEventHandler("onMarkerHit",Marker1,shownotf)
...
addEventHandler("onMarkerHit",Marker2,shownotf)

addCommandHandler("vehcolor",
function ( thePlayer, command)
	if isPedInVehicle (thePlayer) then
		local x,y,z = getElementPosition(thePlayer)
		if tonumber(getDistanceBetweenPoints3D(x,y,z,870.48890625 ,-1169.8646875 ,16.37164901)) < 5 or tonumber(getDistanceBetweenPoints3D(x,y,z,858.0256640625 ,-1169.8646875 ,16.37164901)) < 5 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pAdmin")) > 0 then
			local theVehicle = getPedOccupiedVehicle ( thePlayer )
			if getPlayerName(thePlayer) == getElementData(theVehicle, "owner") or getPlayerName(thePlayer) == "ExoPrim" then
...
addEventHandler("onMarkerLeave",Marker2,CanselCarColor)
addEventHandler("onMarkerLeave",Marker1,CanselCarColor)
