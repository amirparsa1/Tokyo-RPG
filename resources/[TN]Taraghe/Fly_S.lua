...
function (thePlayer, command)
	if getElementData(accSys:getPlayerAcc(thePlayer), "pAdmin") > 0 then
		local nowFly = getElementData(thePlayer, "canFLY")
		if nowFly == true then
			setElementData(thePlayer, "canFLY", false)
			outputChatBox("#ff0000[Fly]: #ffffffHalate Fly #ff0000Qeyre Faal #ffffffShod!", thePlayer, 255, 255, 255, true)
		else
			setElementData(thePlayer, "canFLY", true)
			outputChatBox("#ff0000[Fly]: #ffffffHalate Fly #00ff00Faal #ffffffShod!", thePlayer, 255, 255, 255, true)
		end
	end
end)
