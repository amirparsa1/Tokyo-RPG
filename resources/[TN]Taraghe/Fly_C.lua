...
function Superman.Start()
  ...
  -- Bind keys
    ...
    addCommandHandler("superman", Superman.onJump)
  ...
  --Check right
  triggerServerEvent("superman:checkRight",localPlayer,localPlayer)
end
...
function Superman.onJump(key, keyState)
	if getElementData(getLocalPlayer(), "canFLY") == true then
		local task = getPedSimplestTask(localPlayer)
		if task == "TASK_SIMPLE_IN_AIR" then
			triggerServerEvent("bbinadmine",getLocalPlayer(),getLocalPlayer())
		end
	end
end
...
-- Commands
--
function Superman.cmdSuperman(root)
...
addEvent("flyrookon",true)
addEventHandler("flyrookon", root, Superman.cmdSuperman)
...
player = localPlayer
function jump()
if getElementData(getLocalPlayer(),"derbymode") == true then
if isPedInVehicle(player) then
local veh = getPedOccupiedVehicle(player)
if isPedOnGround(player) then
	local vx,vy,vz = getElementVelocity(veh)
	setElementVelocity(veh,vx,vy,vz + 0.40)--0.40
end
	end
end
end
...
if getElementData(getLocalPlayer(),"derbymode") == true then
	...
	if(vehicle) and not isCursorShowing() then 
		...
		--addEventHandler("onClientPreRender",root,pb)
		setTimer ( function()
		unbindKey("mouse1", "down", shoot1 )
		setTimer(bindkonshoot1, 2900, 1)
	end, 50, 1 )
	end
end
end
...
if getElementData(getLocalPlayer(),"derbymode") == true then
	...
	if(vehicle) and not isCursorShowing() then 
		local x, y, z = getElementPosition(vehicle)
		createProjectile(vehicle, 19, x, y, z)
		--addEventHandler("onClientPreRender",root,pb2)
		setTimer ( function()
        unbindKey("mouse2", "down", shoot2 )
		setTimer(bindkonshoot2, 2900, 1)
	end, 50, 1 )
	end
end
end
...
function bindkonjump()
	bindKey("lshift", "down", jump)
end

bindkonjump()

function bindkonshoot1()
	bindKey("mouse1", "down", shoot1)
end

bindkonshoot1()

function bindkonshoot2()
...
--[[
	local TestPed = createPed ( 120, -1997.515625 ,-960.9931640625 ,32.164207458496 )
	setPedControlState(TestPed, "forwards", true)
	setPedControlState(TestPed, "walk", true)
]]

function glue()
	local player = getLocalPlayer()
	if not getElementData(player, "Staff") then
		outputChatBox("#ff0000Error: chasbam Nemiad :(", 255, 255, 255, true)
		return
	end
...
addCommandHandler("unchasb",unglue)
...
function ( thePlayer )
	if ( thePlayer == getLocalPlayer() ) then
		if CarFly == true then
			...
			outputChatBox("#00ff00[Done]: #ffffffCar Fly #ff0000Off #ffffffShod!", 255, 255, 255, true)
		else
			CarFly = true
			setWorldSpecialPropertyEnabled( "aircars", true )
			setWorldSpecialPropertyEnabled( "hovercars", true )
			outputChatBox("#00ff00[Done]: #ffffffCar Fly #00ff00On #ffffffShod!", 255, 255, 255, true)
		end
	end
end)
