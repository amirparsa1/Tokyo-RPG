...
addEventHandler("onClientHouseSystemMenueOpen", gMe, function(owner, locked, money, weap1, weap2, weap3, id, rentable, rent, tenands)
	if(locked == nil) then
		...
		guiSetInputMode("no_binds_when_editing")
		showCursor(true)
		local X, Y, Width, Height = getMiddleGuiPosition(500,218)
		Fenster2[1] = guiCreateWindow(X, Y, Width, Height, "Housemenue",false)
		Knopf2[3] = guiCreateButton(0,20,25,29,"X",false,Fenster2[1])
		Knopf2[6] = guiCreateButton(351,171,107,29,"Lock House",false,Fenster2[1])



		Label2[15] = guiCreateLabel(224,41,137,15,"Rentalprice: $0",false,Fenster2[1])
		guiSetFont(Label2[15],"default-bold-small")
		Edit2[2] = guiCreateEdit(241,58,75,24,"",false,Fenster2[1])
		Knopf2[7] = guiCreateButton(224,88,114,27,"Set rentalprice",false,Fenster2[1])
		Label2[16] = guiCreateLabel(225,60,20,22,"$",false,Fenster2[1])
		guiSetFont(Label2[16],"default-bold-small")
		Grid2[1] = guiCreateGridList(35,34,184,175,false,Fenster2[1])
		guiGridListSetSelectionMode(Grid2[1],1)

		guiGridListAddColumn(Grid2[1],"Tenant",0.8)
		Knopf2[8] = guiCreateButton(222,171,115,30,"Remove tenant",false,Fenster2[1])
		Checkbox2[1] = guiCreateCheckBox(225,116,111,17,"Are you sure?",false,false,Fenster2[1])
		guiSetFont(Checkbox2[1],"default-bold-small")
		Knopf2[9] = guiCreateButton(223,137,115,30,"Set rentable",false,Fenster2[1])
		-- SET THE SETTINGS --
		-- TENANDS --
		guiGridListClear(Grid2[1])
		...
		addEventHandler("onClientGUIClick", Knopf2[8], function()
			if(owner == false) then outputChatBox("Shoma Saheb In Khane Nistid!", 255, 0, 0) return end
		...
		addEventHandler("onClientGUIClick", Knopf2[7], function()
			...
			if(value == nil) or(value < 1) or (value > 10000) then outputChatBox("Bad input!", 255, 0, 0) return end
...
addEventHandler("onClientHouseSystemGUIStart", gMe, function()
	...
	addEventHandler("onClientGUIClick", Knopf[3], function()
		...
		if(price < 0) or (price > 99999999) then outputChatBox("Bad Price!", 255, 0, 0) return end
	...
	for index, house in pairs(streamedHouses) do
		if(house ~= false) and (house ~= nil) then
			if(isElement(house)) then
				...
				if tonumber(getDistanceBetweenPoints3D(x,y,z, x2, y2, z2)) < 15 then
