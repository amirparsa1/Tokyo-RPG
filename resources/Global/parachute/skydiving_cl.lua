...
local function onRender()
	...
	if tickDiff > 0 then
		if ( not isPedInVehicle( localPlayer ) ) and ( getPedWeapon ( localPlayer, 11 ) == 46 ) and ( not getElementData(localPlayer, "parachuting") ) and ( not doesPedHaveJetPack(localPlayer) ) and ( not isElementAttached(localPlayer) ) and ( not isPedDead(localPlayer) ) then
			...
			and not isElementInWater(localPlayer) and not testLineAgainstWater ( x,y,z,x,y,z + 12) then	
				...
				if not getElementData(localPlayer,"skydiving") then
					if not processLineOfSight ( x,y,z, x,y,z-MIN_GROUND_HEIGHT, true, true,false,true,true,false,false,false,localPlayer ) and not testLineAgainstWater ( x,y,z - MIN_GROUND_HEIGHT,x,y,z) then
						...
						if currentTick > (divingTick + t(600)) then
...
function warningText()
	warning = true
	dxDrawText ( "Press < Left Click > For Open Parachute ", 
