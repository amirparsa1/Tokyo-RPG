	...
	if tickDiff > 0 then
		if ( getElementData ( localPlayer, "parachuting" ) ) then
			if ( changeVelocity ) then
				...
				if ( not isPedOnGround ( localPlayer ) and not getPedContactElement ( localPlayer ) and velZ ~= 0 and not isPedInVehicle ( localPlayer ) ) then
...
function removeParachute(player,type)
	...
	setTimer ( setPedAnimationDelayed, t(600), 1, player )
	openingChutes[chute] = nil
	if chute then
		if type == "land" then
			Animation.createAndPlay(
			  chute,
			  {{ from = 0, to = 100, time = t(600), fn = animationParachute_land }}
			)
			setTimer ( function()
				detachElements ( chute, player )
				setTimer ( destroyElement, t(1000), 1, chute )
			end,
			t(1000),
			1
			)
		elseif type == "water" then
			Animation.createAndPlay(
			  chute,
			  {{ from = 0, to = 100, time = t(600), fn = animationParachute_landOnWater }}
			)
			setTimer ( destroyElement, t(1000), 1, chute )
		end
	end
