...
local ICON_WIDTH = 0.12*g_screenX
...
function dxDrawVoiceLabel ( player, index, color )
	...
	px = px + spacing
	if getElementData(player,"NameTagIsShow?") == false then
		-- shadows
		...
		-- shadows
		dxDrawText ( "HIDE", px + 1, py + 1, px, py, ICON_TEXT_SHADOW, scale )
		-- text
		dxDrawText ("HIDE", px, py, px, py, tocolor(255,255,255), scale )
