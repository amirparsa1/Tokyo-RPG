...
function colorPicker.create(id, start, title)
  ...
  cp.gui.window = guiCreateWindow((sw-cp.gui.width)/1, (sh-cp.gui.height)/2.5, cp.gui.width, cp.gui.height, tostring(title or "COLORPICKER"), false)
  cp.gui.svmap = guiCreateStaticImage(16, 32, 256, 256, "blank.png", false, cp.gui.window)
  cp.gui.hbar = guiCreateStaticImage(288, 32, 32, 256, "blank.png", false, cp.gui.window)
  cp.gui.blank = guiCreateStaticImage(336, 32, 64, 64, "blank.png", false, cp.gui.window)
  ...
  cp.gui.okb = guiCreateButton(336, 263, 64, 28, "OK", false, cp.gui.window)
  cp.gui.closeb = guiCreateButton(336, 295, 64, 28, "Cancel", false, cp.gui.window)
  guiSetProperty(cp.gui.okb, "HoverTextColour", "FF00FF00")
  guiSetProperty(cp.gui.closeb, "HoverTextColour", "FFFE0000")
  guiWindowSetSizable(cp.gui.window, false)	
  Color1 = guiCreateRadioButton(16, 290, 80, 15, "Color 1", false, cp.gui.window)
  labalColor1 = guiCreateLabel(20, 306, 64, 20, "10,000$", false, cp.gui.window)
  labalColor2 = guiCreateLabel(115, 306, 64, 20, "6,000$", false, cp.gui.window)
  labalColor3 = guiCreateLabel(218, 306, 64, 20, "4,000$", false, cp.gui.window)
  guiLabelSetColor(labalColor1, 41, 163, 41)
  guiLabelSetColor(labalColor2, 41, 163, 41)
  guiLabelSetColor(labalColor3, 41, 163, 41)
  Color2 = guiCreateRadioButton(106, 290, 80, 15, "Color 2", false, cp.gui.window)
  Color3 = guiCreateRadioButton(200, 290, 80, 15, "Light Color", false, cp.gui.window)
  guiRadioButtonSetSelected(Color1, true)

  guiSetProperty(cp.gui.window, "Visible", "True")
  ...
  cp.handlers.pickColor = function() cp:pickColor() killTimer(LocalTimer) end
  cp.handlers.destroy = function() cp:destroy() killTimer(LocalTimer) end

  

  function DataChangeColorCar ()
    local windowState = guiGetProperty(cp.gui.window, "Visible")
    if windowState == "True" then
        if(guiRadioButtonGetSelected(Color1)) then
        setElementData(getLocalPlayer() ,"VaziatCpickerForCar", 1)
        elseif (guiRadioButtonGetSelected(Color2)) then
          setElementData(getLocalPlayer() ,"VaziatCpickerForCar", 2)
        elseif (guiRadioButtonGetSelected(Color3)) then
          setElementData(getLocalPlayer() ,"VaziatCpickerForCar", 3)
        end
    end
  end
  LocalTimer = setTimer(DataChangeColorCar, 500, 0)
