...
function colorPicker:mouseUp(button, state)
  if not state or state ~= "down" then
    if self.gui.track then 
      triggerEvent("onColorPickerChanges", root, self.id, self.color.hex, self.color.r, self.color.g, self.color.b) 
    end
...
function colorPicker:pickColor()
  triggerEvent("onColorPickerOK2", root, self.id, self.color.hex, self.color.r, self.color.g, self.color.b)
  self:destroy()
end
...
addEvent("onColorPickerOK2", true)
