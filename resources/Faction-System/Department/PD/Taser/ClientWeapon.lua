...
cFunc["shot_weapon"] = function(hitX, hitY, hitZ, x, y, z)
	playSound3D("Department/PD/Taser/data/Fire.wav", x, y, z)
	local s = playSound3D("Department/PD/Taser/data/Fire.wav", hitX, hitY, hitZ)
...
if not isPedInVehicle(source) then
	if(wep == 23)  and (bodypart == 9) then
		...
		setElementData(source, "Shokers", 5)
	...
	elseif(wep == 23)  and (bodypart == 2) then
		...
		setTimer(
		function ()
		triggerServerEvent("TesteGuinCC2", getLocalPlayer(), getLocalPlayer() )
		end, 10000, 1)
