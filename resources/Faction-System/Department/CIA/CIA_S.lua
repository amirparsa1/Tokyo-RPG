...
function CIAMassege(systemname,Message)
	for index, getAdmins in ipairs( getElementsByType("player") ) do 
		if getElementData(getAdmins, "loggedIn") == true then
			if tonumber(getElementData(accSys:getPlayerAcc(getAdmins), "pMember")) == 6 then
...
addCommandHandler("hide",function(thePlayer,CMD,Target)
    if tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) ~= 5 then
	    if tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 6 then
	    			...
	    			if find then
	    				if tonumber(getElementData(accSys:getPlayerAcc(find), "pMember")) == tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) then
	    					if getElementData(find,"NameTagIsShow?") == false then
	    						...
	    						CIAMassege("CIA-Radio","#FFFF00Tavajoh #FFFFFF: #000000"..getPlayerName(find).." #FFFFFFTavasot #000000"..getPlayerName(thePlayer).." #FFFFFFBe Sorat Hide Dar Amad .")
	    					else
	    						setElementData(find,"NameTagIsShow?",false)
	    						setPlayerNametagShowing( find, true )
	    						CIAMassege("CIA-Radio","#FFFF00Tavajoh #FFFFFF: #000000"..getPlayerName(find).." #FFFFFFTavasot #000000"..getPlayerName(thePlayer).." #FFFFFFAz Hide Dar Amad .")
	    					end
	    				else
	    					SendSystemMasage("#FF0000In Fard Ozv Sazman CIA Nist",thePlayer)
	    				end
	    			else
	    	...
	    	SendSystemMasage("#FF0000Baraye Inkar Bayad Ozv Sazman CIA/ICA Bashid",thePlayer)
	    end
    end
end)

local CIAorder = createPickup ( 258.533203125 ,108.818359375 ,1003.21875 , 3, 1242, 0)
setElementInterior ( CIAorder, 10 )
setElementDimension ( CIAorder, 1 )
local CIAduty = createPickup ( 258.533203125 ,110.818359375 ,1003.21875 , 3, 1275, 0)
setElementInterior ( CIAduty, 10 )
setElementDimension ( CIAduty, 1 )
--/order 
addCommandHandler("order",
function(thePlayer, cmd, orderID)
	local x,y,z = getElementPosition(thePlayer)
	local mx,my,mz = getElementPosition(CIAorder)
	...
	if tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 1 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 2 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 3 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 6 then
		if de < fasele and getElementDimension(thePlayer) == 1 and getElementInterior(thePlayer) == 10 then
...
function(thePlayer, cmd, ...)
	local x,y,z = getElementPosition(thePlayer)
	local mx,my,mz = getElementPosition(CIAduty)
	local de = getDistanceBetweenPoints3D(x,y,z,mx,my,mz)
	if tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 1 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 2 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 3 or tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "pMember")) == 6 then
		if de < fasele and getElementDimension(thePlayer) == 1 and getElementInterior(thePlayer) == 10 then
