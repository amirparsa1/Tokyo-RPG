...
Minimap.CurrentZoom = 6;
Minimap.MaximumZoom = 6;
Minimap.MinimumZoom = 6;
	...
	function()
		...
		if (not Minimap.IsVisible and Bigmap.IsVisible) then
			...
			if (getElementInterior(localPlayer) == 0) then
				...
				dxDrawText('Inja GPS Anten Nmide :(', Bigmap.PosX, Bigmap.PosY + 20, Bigmap.PosX + Bigmap.Width, Bigmap.PosY + 20 + Bigmap.Height, tocolor(255, 255, 255, 255 * math.abs(getTickCount() % 2000 - 1000) / 1000), 0.40, Fonts.Roboto, 'center', 'center', false, false, false, true, true);
		...
		elseif (Minimap.IsVisible and not Bigmap.IsVisible and visible2) then
			...
			if (getElementInterior(localPlayer) == 0) then
				...
				--> vehicles
					if getElementInterior(localPlayer) == 0 and mycolshape then
					
					for _, veh in ipairs(getElementsWithinColShape ( mycolshape, "vehicle" ) ) do
					local blipX, blipY, blipZ = getElementPosition(veh);
					if isVehicleEmpty( veh ) then
						...
						-- dxDrawImage(blipX, blipY, blipSettings['size'], blipSettings['size'], 'files/images/blips/veh.png', 0, 0, 0, tocolor(255,255,255,255), false);
						dxSetBlendMode('blend');
					end
					end	
					end
				...
				if getElementInterior(localPlayer) == 0 and getElementDimension(localPlayer) == 0 and mycolobj then
					
					for koon, obj in ipairs(getElementsWithinColShape ( mycolobj, "object" ) ) do
					if isobjmark[obj] then
					--outputDebugString("khob... "..koon)
					local blipX, blipY, blipZ = getElementPosition(obj);
						...
						dxDrawImage(blipX, blipY, blipSettings['size'], blipSettings['size'], 'files/images/blips/object.png', 0, 0, 0, tocolor(255,255,255,255), false);
						dxSetBlendMode('blend');
				...
				--> Local player
				dxDrawImage((Minimap.PosX + (Minimap.Width / 2)) -10, (Minimap.PosY + (Minimap.Height / 2)) - 10, 22, 22, 'files/images/arrow.png', math.deg(-pRotation) - playerRotation);
				...
				dxDrawText('Inja GPS Anten Nmide :(', Minimap.PosX, Minimap.PosY + 20, Minimap.PosX + Minimap.Width, Minimap.PosY + 20 + Minimap.Height, tocolor(255, 255, 255, 255 * math.abs(getTickCount() % 2000 - 1000) / 1000), 0.40, Fonts.Roboto, 'center', 'center', false, false, false, true, true);
...
local x,y,z = getElementPosition(localPlayer)
	mycolshape = createColSphere ( x,y,z, 200 )
	attachElements ( mycolshape, localPlayer )
function showMinimap(bool)
visible2 = bool
...
function()
	if (not Minimap.IsVisible and Bigmap.IsVisible) then
		...
		local mapX, mapY = getWorldFromMapPosition(cursorX, cursorY);
		zoneName = getZoneName(mapX, mapY, 0);
		if markedPlace then destroyElement(markedPlace) markedPlace = nil return end
			markedPlace = createBlip(mapX, mapY, 0, 41)
