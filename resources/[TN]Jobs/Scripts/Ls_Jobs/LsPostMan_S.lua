...
local LsPostManGetJob = createMarker(-51.207801818848 ,-1391.4719238281 ,10.418999671936 , "cylinder", 1.0,0,221,221,211)
addEventHandler("onMarkerHit",LsPostManGetJob,
...
local LsPVehicle = {}
local LsPNowPostMarker1ID = {}
local LsPNowPostMarker1 = {}
local LsPNowPostMarker2ID = {}
local LsPNowPostMarker2 = {}
local LsPNowPostBlip = {}
...
function ( thePlayer, seat, jacked ) 
	...
	if vehid == "PM1" or vehid == "PM2" or vehid == "PM3" or vehid == "PM4" or vehid == "PM5" or vehid == "PM6" or vehid == "PM7" or vehid == "PM8" or vehid == "PM9" or vehid == "PM10" then
		if seat == 0 then
			if not LsPNowPostMarker1[thePlayer] then
				if not LsPVehicle[thePlayer] then
					LsPVehicle[thePlayer] = source
				end
				local Shansi = math.random( 1 , 50 )
				LsPNowPostMarker1[thePlayer] = createMarker(LsPostLocations[Shansi][1],LsPostLocations[Shansi][2],LsPostLocations[Shansi][3]-2,"checkpoint",3,255, 255, 0,200, thePlayer)
				LsPNowPostMarker1ID[LsPNowPostMarker1[thePlayer]] = Shansi
				LsPNowPostBlip[thePlayer] = createBlipAttachedTo(LsPNowPostMarker1[thePlayer], 56, 2, 255, 0, 0, 255,  0 , 65535,thePlayer)
			end
		end
	end
end)
...
function (hitElement,matchingDimension)
	if (getElementType(hitElement) == "player" and tonumber(getElementData(accSys:getPlayerAcc(hitElement),"pJob")) == 8 and isPedInVehicle(hitElement))  then
		if LsPNowPostMarker1[hitElement] == source then
			local vehicle = getPedOccupiedVehicle(hitElement)
			if LsPVehicle[hitElement] == vehicle then
				setElementFrozen(LsPVehicle[hitElement] ,true)
				local Shansi = LsPNowPostMarker1ID[LsPNowPostMarker1[hitElement]] 
				LsPNowPostMarker1ID[LsPNowPostMarker1[hitElement]] = nil
				destroyElement(LsPNowPostMarker1[hitElement]) 
				LsPNowPostMarker1[hitElement] = nil
				destroyElement(LsPNowPostBlip[hitElement]) 
				LsPNowPostBlip[hitElement] = nil
				LsPNowPostMarker2[hitElement] = createMarker(LsPostLocations[Shansi][4],LsPostLocations[Shansi][5],LsPostLocations[Shansi][6]-2,"checkpoint",2,255, 255, 0,200, hitElement)
				LsPNowPostBlip[hitElement] = createBlipAttachedTo(LsPNowPostMarker2[hitElement], 56, 2, 255, 0, 0, 255,  0 , 65535,hitElement)
...
function (hitElement,matchingDimension)
	if (getElementType(hitElement) == "player" and tonumber(getElementData(accSys:getPlayerAcc(hitElement),"pJob")) == 8 )  then
		if LsPNowPostMarker2[hitElement] == source then
			setElementFrozen(hitElement,true)
			destroyElement(LsPNowPostMarker2[hitElement])
			LsPNowPostMarker2[hitElement] = nil
			destroyElement(LsPNowPostBlip[hitElement]) 
			LsPNowPostBlip[hitElement] = nil
			...
			exports["notf"]:addNotification(hitElement, "Dar Hale Tahvile Name , Sabr Konid ..." , 'info')
				setTimer(function()
					...
					if LsPVehicle[hitElement] then
						setElementFrozen(LsPVehicle[hitElement] ,false)
					end
					local mablagh = 0
...
function()
	if LsPNowPostMarker1[source] then
		destroyElement(LsPNowPostMarker1[source]) 
		LsPNowPostMarker1[source] = nil
		destroyElement(LsPNowPostBlip[source]) 
		LsPNowPostBlip[source] = nil
	end
	if LsPNowPostMarker2[source] then
		destroyElement(LsPNowPostMarker2[source]) 
		LsPNowPostMarker2[source] = nil
		destroyElement(LsPNowPostBlip[source]) 
		LsPNowPostBlip[source] = nil
	end
	if LsPVehicle[source] then
		LsPVehicle[source] = nil
		setElementFrozen(LsPVehicle[source],false)
	end
end)
...
function ()
	...
	or vehid == "PM6" or vehid == "PM7" or vehid == "PM8" or vehid == "PM9" or vehid == "PM10" then
		...
		if find then
			if LsPVehicle[find] == source then
				if LsPNowPostMarker1[find] then
					destroyElement(LsPNowPostMarker1[find]) 
					LsPNowPostMarker1[find] = nil
					destroyElement(LsPNowPostBlip[find]) 
					LsPNowPostBlip[find] = nil
				end
				if LsPNowPostMarker2[find] then
					destroyElement(LsPNowPostMarker2[find]) 
					LsPNowPostMarker2[find] = nil
					destroyElement(LsPNowPostBlip[find]) 
					LsPNowPostBlip[find] = nil
				end
				if LsPVehicle[find] then
					LsPVehicle[find] = nil
				end
...
function (thePlayer,seat)
	if LsPVehicle[thePlayer] == source then
		if seat == 0 then
			if not isElementFrozen(source) then
				exports["[TN]Jobs"]:RespawnJobVehOnExit ( LsPVehicle[thePlayer] )
				if LsPNowPostMarker2[thePlayer] then
					destroyElement(LsPNowPostMarker2[thePlayer]) 
					LsPNowPostMarker2[thePlayer] = nil
					destroyElement(LsPNowPostBlip[thePlayer]) 
					LsPNowPostBlip[thePlayer] = nil
				end
				if LsPNowPostMarker1[thePlayer] then
					destroyElement(LsPNowPostMarker1[thePlayer]) 
					LsPNowPostMarker1[thePlayer] = nil
					destroyElement(LsPNowPostBlip[thePlayer]) 
					LsPNowPostBlip[thePlayer] = nil
				end
				if LsPVehicle[thePlayer] then
					LsPVehicle[thePlayer] = nil
				end
			end
		end
	end
...
function ( )
	...
	if LsPVehicle[source] then
		...
		respawnVehicle(LsPVehicle[source])
		LsPVehicle[source] = nil
	end
end)
